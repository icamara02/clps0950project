...
idx = strcmp(raw_data.resultId, '650352b0938c4e060cb94494') & strcmp(raw_data.Congruence, 'GC_Cong');%finds the index of every time it says that participant (cld be pp)
newtable = raw_data(idx, :); %runs through raw data and only outputs when ^ is true
average = mean([newtable.responseTime]);
%put this in the table

for dd = 1:size(pivot_table,1) %go through the row
   pid = pivot_table(dd,1);
   idx = strcmp(raw_data.resultId, pid{1}); %& strcmp(raw_data.Congruence, 'GC_Cong');
   conditiontable = raw_data(idx, :);
